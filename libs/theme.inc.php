   ...
   <li id="li-comment-<?php comment_ID() ?>">
     <div class="clearfix" id="comment-<?php comment_ID(); ?>">
        <div class="comment_area">
           ...
           <p class="comment_author"><?php comment_author_link() ?> <span class="comment_date">&bull; <a href="#comment-<?php comment_ID() ?>"><?php if(!function_exists('how_long_ago')){comment_date() . ' @ ' . comment_time(); } else { echo how_long_ago(get_comment_time('U')); } ?></span> <span class="comment_date"></span></a></p>
	       <?php comment_text() ?>
...
function mm_content_class() {
    ...
    //if(is_single()) { echo ' class="single"';} else if(is_home()) { echo ' class="home"';}
...
function mm_archive_navigation(){
  if (show_posts_nav()){
    ...
    new_previous_posts_link('<i class="fa fa-angle-left"></i> Newer');
    new_next_posts_link('Older <i class="fa fa-angle-right"></i>');
    echo '</nav>';
  }
}
...
function mm_single_navigation(){
    ...
    new_previous_post_link('%link','<i class="fa fa-angle-left"></i> Older');
    new_next_post_link('%link','Newer <i class="fa fa-angle-right"></i>');
    echo '</nav>';
}

function mm_portfolio_navigation(){
    new_previous_post_link('%link','<i class="fa fa-angle-left"></i>', TRUE, $classcall='prev');
    new_next_post_link('%link','<i class="fa fa-angle-right"></i>', TRUE, $classcall='next');
}

function listYear($a) {
    $currentyear = date('Y'); 
    if($currentyear!=$a) {
        $theyear = $a . "&ndash;" . $currentyear;
    } else{
        $theyear = $currentyear;
    } 
  echo"$theyear";
